...
import { Text, View, SafeAreaView, ScrollView, StyleSheet, TouchableOpacity, ActivityIndicator, BackHandler, Image } from 'react-native';
...
const Directory = ({ handlePress, showTheDirectory }) => {
  ...
  useEffect(() => {
    setIsLoading(true);
    axios.get(`http://89.117.36.161/api/dinos?sorted=1`, {
      headers: {
  ...
  return (
    <SafeAreaView style={dinos.length && styles.directoryContainer}>
      ...
      >
        {
          dinos.map(dino => (
            <TouchableOpacity key={dino?._id} style={styles.dinoCard} onPress={() => handlePress(dino)}>
              <Image
        ...
        {(isLoading && currentPage > 0) ? <ActivityIndicator size="large" color="#ffffff" /> : null}
      </ScrollView>
    </SafeAreaView>
  );
}
